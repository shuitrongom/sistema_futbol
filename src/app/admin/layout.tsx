...
function SidebarContent({ pathname, adminName, onNavigate }: { pathname: string; adminName: string; onNavigate?: () => void }) {
  return (
      ...
      <div className="p-5 border-b border-[#C1D82F]/10">
        <Link href="/admin/dashboard" className="flex items-center gap-3 group" onClick={onNavigate}>
          <div className="size-10 rounded-xl bg-gradient-to-br from-[#C1D82F] to-[#2B8B41] flex items-center justify-center shadow-lg shadow-[#C1D82F]/20">
            <Settings className="size-5 text-[#1a1f36]" />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-bold tracking-wide text-[#C1D82F]">ADMIN</p>
            <p className="text-[11px] text-white/40 truncate">{adminName || "Administrador"}</p>
          </div>
        </Link>
      </div>
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await fetch("/api/auth/session"); const s = await r.json(); if (s?.user?.name) setAdminName(s.user.name); } catch {}
    })();
  }, []);
    ...
    <div className="flex h-screen overflow-hidden bg-[#0d1117]">
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="p-0 w-64 border-0 bg-transparent" showCloseButton={false}>
          <SidebarContent pathname={pathname} adminName={adminName} onNavigate={() => setMobileOpen(false)} />
        </SheetContent>
      </Sheet>
      <main className="flex-1 overflow-y-auto bg-[#0d1117]">
        <div className="lg:hidden sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-[#1a1f36] border-b border-[#C1D82F]/10">
          ...
          <span className="text-[#C1D82F] font-bold text-sm tracking-wide">ADMIN</span>
        </div>
